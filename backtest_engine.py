...
=============================================================================
  ...
  Fixed: yfinance data fetch, date range handling, Vercel compatibility
=============================================================================
"""
...
# =============================================================================
# DATA FETCH  — Fixed for Vercel (no period=, use start/end dates)
# =============================================================================
def fetch_data(ticker, start, end=None):
    ...
    if len(df) < 60:
        raise ValueError(f"Not enough data for {ticker} (got {len(df)} rows, need 60+).")
...
def add_indicators(df, params):
    ...
    # Bollinger Bands
    bb_p = s.get('bb_period', 20)
    bb_s = s.get('bb_std', 2.0)
...
def strategy_combined(df, params):
    ...
    ob = params.get('rsi_overbought', 70)
    lm = params.get('long_ma', 50)
    ma_long = df['MA_Short'] > df['MA_Long']
    rsi_ok  = df['RSI'] < ob
    pos = []; cur = 0
...
def compute_metrics(df, initial_capital=100_000.0, risk_free_rate=0.05):
    ...
    cagr = ((fv / initial_capital) ** (1 / ny) - 1) * 100
    ...
    if std is None or np.isnan(std) or std < 1e-10:
        ...
        sharpe = float((ex.mean() / std) * np.sqrt(252))
        sharpe = round(max(-50.0, min(50.0, sharpe)), 3)
    ...
    return {
        ...
        "cagr":            round(cagr, 2),
        "sharpe_ratio":    sharpe,
        "max_drawdown":    round(float(maxdd), 2),
...
def generate_charts(df, ticker, metrics, strategy_name='MA Crossover'):
    ...
    ax1.set_title(f'  {ticker} | {strategy_name}  ·  QuantEdge v2.1',
...
if __name__ == '__main__':
    print("\n" + "=" * 65)
    print("  QuantEdge v2.1 — Multi-Strategy Test")
    print("=" * 65)
    for strat in ['ma_crossover', 'rsi', 'macd', 'bollinger', 'combined']:
        r = run_full_backtest('AAPL', '2020-01-01', strategy=strat)
        m = r['metrics']
        print(f"\n  [{r['strategy_label']}]")
        print(f"  Return:{m['total_return']:+.1f}%  Sharpe:{m['sharpe_ratio']:.3f}  "
              f"MaxDD:{m['max_drawdown']:.1f}%  Trades:{m['n_trades']}")
    print("\n" + "=" * 65)
